...
public class DepartamentoController {
	...
	public ResponseEntity<?> obtenerDepartamentos() {
		...
		/*
		 * else { List<DepartamentoDTO> depaDto = depa.stream().map(espec ->
		 * modelMapper.map(espec, DepartamentoDTO.class)) .collect(Collectors.toList());
		 * ApiResponse<List<DepartamentoDTO>> response = new ApiResponse<>(true,
		 * "Lista de Departamentos", depaDto); return ResponseEntity.ok(response); }
		 */

		for (Departamento departamento : depa) {
			departamento.add(linkTo(
					methodOn(DepartamentoController.class).obtenerDepartamentosPorId(departamento.getIdDepartamento()))
					.withSelfRel());
			departamento.add(linkTo(methodOn(DepartamentoController.class).obtenerDepartamentos())
	...
	public ResponseEntity<?> obtenerDepartamentosPorId(@PathVariable Long id) throws EntityNotFoundException {
		...
		dpto.add(linkTo(methodOn(DepartamentoController.class).obtenerDepartamentosPorId(dpto.getIdDepartamento()))
				.withSelfRel());
	 ...
	 * @throws EntityNotFoundException   Si no se encuentra el departamento o la
	 *                                   especialidad.
	 * @throws IllegalOperationException Si se produce una operación ilegal.
	 */
	...
	public ResponseEntity<?> asignarEspecialidad(@PathVariable Long IdDpto, @PathVariable Long IdEsp)
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body("Operación ilegal durante la asignación de especialidad");
		} catch (Exception e) {
